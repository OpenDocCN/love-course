...
*   最后的条件语句确保当脚本直接运行时，Batre的命令解析器会被调用。
...
1.  获取帮助信息：在终端运行 `python app.py --help`，你会看到自动生成的帮助文档，其中列出了 `hello` 命令及其参数说明。
2.  执行命令：运行 `python app.py hello --name World`，终端将输出 `Hello, World!`。
3.  使用缩写：Batre通常支持参数缩写，你也可以运行 `python app.py hello -n World` 达到同样效果。

## 总结 📝

本节课中我们一起学习了Batre库的基础知识。我们了解了Batre是一个用于构建CLI的工具，学会了通过 `@command` 和 `@argument` 装饰器来定义命令和参数，并成功创建并运行了一个简单的命令行应用。掌握这些基础后，你可以继续探索Batre的更高级功能，如子命令、参数类型验证和复杂的选项配置，以构建更强大的命令行工具。